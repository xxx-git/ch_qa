...
tf.app.flags.DEFINE_float("learning_rate_decay_factor", config.learning_rate_decay_factor,
...
tf.app.flags.DEFINE_integer("max_vocab_size", config.max_vocab_size, "max vocabulary size.")
...
tf.app.flags.DEFINE_integer("src_vocab_size", config.src_vocab_size, "source vocabulary size.")
tf.app.flags.DEFINE_integer("dest_vocab_size", config.dest_vocab_size, "destination vocabulary size.")
...
def create_model(session, forward_only):
    ...
    if ckpt:
        model_checkpoint_path = os.path.join(train_dir, ckpt.model_checkpoint_path)
...
def train():
    ...
    src_train, dest_train, src_dev, dest_dev, src_vocab_size, dest_vocab_size, \
    _, _ = data_utils.prepare_headline_data(FLAGS.data_dir, FLAGS.max_vocab_size)

    # device config for CPU usage
    config = tf.ConfigProto()  # n threads parallel for ops
    # tf.device('/gpu:0')
    with tf.Session(config=config) as sess:
        ...
        while True:
            ...
            if current_step % FLAGS.steps_per_checkpoint == 0:
                ...
                model.saver.save(sess, "headline_large.ckpt", global_step=model.global_step)
                step_time, loss = 0.0, 0.0
